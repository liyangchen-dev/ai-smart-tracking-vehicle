...
def run():
    ...
    parser.add_argument('--tcp_addr', type=str, help='TCP address',default='192.168.1.1:2002')
    parser.add_argument('--camera_addr', type=str, help='Camera address',default='http://192.168.1.1:8080/?action=stream')
    args = parser.parse_args()
    tcp_addr = args.tcp_addr
    try:
        print('tcp_addr: ', tcp_addr)
        tcp_server = tcp_addr.split(':')[0]
        tcp_port = int(tcp_addr.split(':')[1])
    except:
        print(traceback.format_exc())
        sys.exit()
    camera_addr = args.camera_addr
    try:
        print('camera_addr: ', camera_addr)
        if len(camera_addr) == 1:
    ...
    #
    ThreadingList.append(
        threading.Thread(target=receive_frame, args=(cap,), daemon=True, name='receive_frame'))
    ThreadingList.append(
        threading.Thread(target=convert_command, args=(client_socket, log_path), daemon=True, name='convert_command'))
